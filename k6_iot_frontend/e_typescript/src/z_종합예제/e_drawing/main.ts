/*
  ! JS의 canvas 요소
  : HTML의 <canvas> 태그를 이용해서 Javascript로 동적인 그래픽을 그리는 기술
...
// EX) key : 'color' , value : 'string' (ToolState color 타입)
...
function createToolbar(): HTMLElement {
  ...
  // 지정된 사각형 영역을 지워 투명하게 만드는 기능(x축 시작, y축 시작)
  ...
  saveButton.onclick = () => {
    ...
    link.download = "drawing.png"; // 저장 파일명 지정
...
canvas.addEventListener("mousemove", (e) => {
  if (!isDrawing) return; // 그리지 않을경우 리턴
  if (ctx) {
    ctx.strokeStyle = toolState.isEraser ? "#fff" : toolState.color;
...
canvas.addEventListener("mouseup", () => {
  isDrawing = false; //그리기 좋류
  ctx?.closePath(); // 경로 종료
});

//? 캔버스를 벗어난 경우(뗀경우와 마찬가지로 종료)
