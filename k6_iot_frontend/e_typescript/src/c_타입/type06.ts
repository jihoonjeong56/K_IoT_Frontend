...
//  +) 유니언 타입과 함께 사용하여 다양한 값을 표현함고 ㅏ동시에 제한 가능
//  >> 변수가 '특정 값'들 중 '하나만 가질 수 있도록 제한'

// 1) 변수 사용
type Directions = "up" | "douwn" | "left" | "right";
let move: Directions;
// move = "diagonal"; //대각선 - let move: Directions
move = "douwn";
...
// greet(p1); - Person 타입의 구조와 일치하지 않음 -> Person으로 취급하지 않음
greet(p2); // 구조적 타이핑에 의해  Person 취금(hobby 속성 무시)
greet(p3); // Person과구조가 일치하기 때문에 Person 취급
...
// +) 객체의 인덱스 서명
// : 객체의 모든 속서에 대해 타입을 정의하지 않고, 키와 값의 타입만 정의하여 구조를 유연하게 적용하는 방법

type UserDataType = {
  //? 일반적인 객체 속성 타입 명시
  name: string; // 속성명: 속성타입;

  //? 인뎃스 서명(signature, 시그니처)
  // [속성명: 인덱스타입]: 속성값타입;
