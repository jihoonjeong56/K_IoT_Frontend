...
// - 컴파일러에게 '이 데이터의 타입을 내가 지정한 타입으로 간주래하' 라는 지시
...
someValue = true; // 어떤 값이든 할당 가능하지만 개발자가 문자열로 단언 한고 싶을 경우
...
// - getElementById(), querySelector() 등으로 HTML 요소를 가져올때 리턴받는 DOM 객체의 타입은 HTMLElement 타입
// +) 해당하는 선택자의 요소가 없을 경우 null반환

// cf) HTMLElement 타입은 각 요소들이 가진 고유한 속성에 접근할 수 없음
//    >> 각 DOM 객체가 가지느 고유한 속성에 접근하기 위해서는 반드시 타입 단언이 필요함
...
// 2) JSON 문자열
// - 객체(parse) <> JSOM(stringfy)
// : JSON 문자열을 분석(parse) 하여 반호나되는 데이터는 any 타입으로 간주

const jsonString = '{"name" : "a", "age": 23}';
