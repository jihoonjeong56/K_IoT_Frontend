...
// 2. 비동기 작업을 사용하여 JSONPlaceholer에서 사용자 데이터 가져오는 함수 정의
...
function handleResponse(response: FetchResponse) {
  ...
  if ("error" in response) {
    ...
    console.log(
      `Name: ${response.name}, Email: ${response.email}, Companey: ${response.company.name}`
    );
  }
}
...
fetchUserData(2);
// Name: Ervin Howell, Email: Shanna@melissa.tv, Companey: Deckow-Crist
fetchUserData(222);
// 네트워크 응답이 실패하였습니다.
