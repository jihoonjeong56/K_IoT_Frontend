...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // >> HTML 문서 구조를 인식하ㅗㄱ 해석한 뒤 내용만 삽입
});
...
// 문서객체.setAttibute(속성이름, 값)
// >> 특정 속성에 값을 지정

// 문서객체.getAttibute(속성이름)
// >> 특정 속성값을 추출

document.addEventListener("DOMContentLoaded", () => {
  const dogs = document.querySelectorAll(".dog");
  dogs.forEach((dog, index) => {
    // 배열 메서드 콜백함수의 인자: 각요소, 인덱스번소, 배열 그자체
    ...
    dog.style.height = 250 + "px";

    //? cf) HTML 표준에 정의도니 속성
    //    : setAttribute(), getAttribute()사용
    //    +) 내장된 속성들은 .연산자를 사용하여 속성 릭기 또는 설정 가능
    dog.src = "../tlqk.jpg";
    dog.alt = "시바 ";
  });
});
