...
//! === 이벤트 객체 ===
// : 이벤트 핸들러가 호출될 때, 브라우저가 자동으로 '이벤트 객체'를 생성하여 이벤트 핸들러에게 전달
...
//? 3. currentTarget
// : 이벤트 리스너가 실제로 첨부된 요소르 '참조'

//? 4. preventDefault()
// : 브라우저가 해당 이벤트에 대해 기본적으로 수행하는 동작을 방지
// > 1) a태그의 링크기능 방지
// > 2) form 태그 안의 submit 역할 방지 (새로고침 방지)
...
container.addEventListener("click", (event) => {
  ...
  console.log(
    "이벤트 리스너가 부탁된 요소(currentTarget) :" + event.currentTarget
  );
...
form.onsubmit = (e) => {
  ...
  //? input 요소의 내용값 삽입
  // - placeholder: 힌트삽입(활성화시 사라짐)
  // >> 참조값(요소값).value 속성

  if (fname.value === "" || femail.value === "") {
    // 제출 입력값에 대한 유혀성 검증이 모두 이루어지지않은 경우
    // >> 제출 이벤트에 대한 행동 방지
    e.preventDefault();
...
// +) 이벤트 캡쳐링
// : 최상위 객체(window or document)에서 이벤트 발생 요소 (target)로 내려가는 단계
// > addEventListener의 세 번째 인자를true로 주어야, 캡쳐링 단계에서 이벤트 감지 가능(기본값: false - 버블링)
