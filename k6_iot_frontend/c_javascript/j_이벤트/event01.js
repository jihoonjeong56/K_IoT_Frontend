...
// cf) on 키워드
// : 이벤트 요소를 웹 문서의 요소에 연결시켜주는 키워드
// - HTML 이벤트 핸드러 속성은 on 키워드와 이벤트 사이에 대문자 변환이 발생하지 않는다.
bgButton.onclick = function () {
  const randomColor = randomColorFunc();

  // bgButton: 웹 요소(객체)
  // > HTML 요소 내의 속성을 객체 속성처럼 사용
  // > JS 에서는 스타일규칙을 lowerCamelCase로 사용
...
window.onkeydown = function () {
  keydownPara.textContent = "안녕하세요 반값습니다.";
};
...
// - 한 요소에 여러개의 이벤트 핸들러 등록이 가능

const buttons = document.querySelectorAll(".buttonsChange");

// buttons: 동일한 선택자의 요소들이 배열로 반화
buttons.forEach((btn) => {
  // HTML 요소.addEventListener('이벤트', 콜백함수);
  // > HTML요소에 해당 이베늩가 발생하면 콜백함수를 실행
