...
let wastebasket = []; // 휴지통 (삭제된 할일저장소)
let nextId = 1; // 고유 ID값을 위한 전역변수

// 1) 할일 추가 함수
function addTodo(content) {
  // 함수 호출 시 인자로 할 일의 내용을 전달받음
  // 새로운 할 일 생성
  const newTodo = {
    id: nextId, // 전역변수는 파일 어디서든 접근 가능
    content: content, // 키: 값(매개변수의 데이터 삽입)
    completed: false,
  };
  // 다음 할 일 추가시 사용할 고유 ID값(전역변수) 증가
  nextId++;
  // 할 일을 목록에 추가
  todos.push(newTodo);
  // 현재까지의 todo 리스트 출력
...
// : 수저하고자 하는 특정 할 일의 id를 매개변수로 전달받아 '완료 상태 전환(토글)'
function toggleTodo(id) {
  // 전체 할 일 목록 중 id값과 일치하는 할 일만 완료 상태 전환
  //? map: 배열을 순회하여 동일한 기능 저용 후 새로운 배열에 담아 반환
  //    >> 요소의 개수 변화 없음

  //? 콜백함수를 가지는 매열의 메서드 'forEach, map, filter'
  //  >> 콜백함수의 인자 (value, index, array)
  //      : 순서 중요
  //      Ex) 두번째 인자 index만 사용하는 경우 - (,index)
  todos = todos.map((todo) => {
    // 화살표 함수의 구현부가 여러줄 일 경우에는 중괄호 생략 불가
    ...
    if (todo.id === id) {
      ...
      // let array1 = [1, 2, 3];
      // let array2 = [...array1]; >> array1의 요소만 가져와서새로운 배열 생성(메모리주소가 다름)

      // let obj1 = {a:1, b:2};
      // let obj2 = {...obj1}; >> obj1의 요소만 가져와서새로운 배열 생성(메모리주소가 다름)
...
function deleteTodo(id) {
  ...
  // splice(시작 인텍스, 삭제할 요소의 개수, 추가할 요소) : 제거 / 추가 가능
  // 1) 제거(시작 인텍스, 삭제할 요소의 개수);    >> 제거된 요소가 배열로 반환
  // 2) 추가(시작인텍스, 0, 추가할 요소 나열);
  const [removed] = todos.splice(idx, 1); //! 구조분해 할당 - removed 상수 사용 가능
  wastebasket.push(removed);

  // 1,2,3,4,5 중에서 4를 제거
  // === 4dhk dlfclgkwl dksgsms 1,2,3,5 만을 새로은 배열로 저장
  // >> 4가 삭제된 효과
