...
//    resolve(value): 프롬니스 이행 상태로 변경, 결과값은 value로 반환
//    reject(error): 프로미스 실패 상태로 변경, 결과값은 error반환
...
//! 프로미스 메서드 사용(호출)
// : new Promise()를 통해 싱성된 프로미스 객체 내부의 메서드
...
function fetchUserData() {
  return new Promise((resolve, reject) => {
    setTimeout(() => resolve("데이터가져오기 성공"), 2000);
  });
}

// '데이터 처리하기;
function processUserDate(data) {
  return new Promise((resolve, reject) => {
    setTimeout(() => resolve(`${data} 사용하기`), 2000);
  });
}
fetchUserData()
  .then((result) => processUserDate(result))
  .then((processResult) => console.log(processResult))
  .catch((error) => console.error(error));

//? console.error(): 웹 콘솔에 에러 메세시 출력  - cer
