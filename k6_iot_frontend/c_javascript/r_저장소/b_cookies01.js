...
// 1) 쿠키 생성 및 수정
// document.cookie = '쿠키이름=쿠키값; expire= 날짜; path=경로; domain=도메인';
// - 쿠키이름, 쿠키값 (필수)
// - 나머지(선택)
document.cookie = "username=aaa; path=/;";
...
// >> 만료 시간 후에도 메모리사에는 잠시 남아있을 수 있음

// 3) 쿠키 읽기
// : 쿠키는 하나의 문자열로 반환
console.log(document.cookie);
// username=aaa; user; username=aaa; userEmail=qwe123
// >> 문자열이기 때문에 직접 파싱
...
function getCookie(name) {
  ...
  // >> 이러터블(순회할 수 있는 자료형: 배열, 문자열, Map, Set 등)
  // >> 실제 요소 값을 순회
  // >> 사용 권장 대상: 배열, 문자열 등

  for (let cookie of cookies) {
    let [key, value] = cookie.split("="); // 구조분해 할당
