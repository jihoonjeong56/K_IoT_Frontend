...
function addTodo() {
  ...
  if (todoInput.value !== "") {
    ...
    // 요소에 class 속성 추가
    // : li 태그는 이벤트에 따라 동적으로 여러 개 생성 가능
    ...
    //? input 등과 같이 상호작용 도니 데이터는 사용 후 해당 데이터 초기화
    todoInput.value = "";
  }
}
...
todoList.addEventListener("click", (e) => {
  ...
  // : todoItem 클릭 시 todoItem 내용이 클자 디자인 변경
  ...
  // +) DOM 요소의 태그명 가져오기
  // : HTML 요소.tagName
  // : 태그명이 알파벳이 모두 대문자 표기(A, LI, DIV 등)
  if (e.target.tagName === "LI") {
    // HTML요소.classList: class 속송에 접근
    // cf) .toggle('클래스명')속성
    // : classList에서 goekd zmffotmauddl xhrmf
    // - 있으면 '삭제'
    // - 없으면 '추가'
