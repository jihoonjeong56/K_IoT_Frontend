...
//! 1번 문제
// 다음 배열을 조건에 따라 작성하고 실행 했을때의 결과를 확인하시오

const scores = [75, 83, 95, 64, 100, 55, 23];
...
//   const addten = numbers.map((a) => a + 10).filter((num) => num % 2 !== 0);
//   return [addten];
// }
// const numResult = sortNum(numbers);
// console.log(numResult);

// const [sortedNum] = numResult;
...
const filteredWords = (words, substring) => {
  // 1) 첫 번째 인자: 배열
  // 2) 두번째 인자 : 필터링할 단어
  // >> word(배열)의 각 단어중 substring(문자열)이 포함된 단어만 필터링하여 반환

  // +) includes() 메더스 활용: 문자열.includes(문자)
  //      >> 문자열 안에 인자로 전달된 문자가 포함되어 있을경우 true, 그렇지 않으면 false
