...
//? 1) 구문 오류
// : 프로그램 실행 전에 발생
// - 코드 작성 시 문법이 언어에서 정의된 규칙을 다르지 않을 때 발생
...
//? 2) 런타임 오류
// : 프로그램 실행 붕 발생 (Node.js 환경 OR 브라우저 환경)
// - 예측가능한 비정상적인 조건 또는 예외적인 사건을 의미
...
// cf) JS에서는 SyntaxError로 출력되는 오류 이외의 모든 오류가 예외(런타임 에러)로 분류
// >> TypeError, ReferenceError, RangeError

//! 예외 처리방법

// 1) 기본 예외 처리: 조건문 사용 - 권장 안함
...
try {
  ...
  // - message: 사람이 읽을수 있는 형태의 에러 내용이 담김
  // - stack: 에러가 발생한 지점의 호출 스택을 문자열로 반환
  console.log(e.message);
} finally {
  console.log("언제나 반드시 실행");
}
