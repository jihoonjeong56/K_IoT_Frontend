...
//    > 매번 API 호줄 하지 않고, 한 번 불러온 데이터를 전역에 저장하고 사용(효율적)
...
// 2) 전역 로딩 상태 관리 - 한 번 데이터가 로딩 되었는지 여부를 저장(boolean)
//  > 여러 컴포넌트들이 isLoaded 데이터를 보고 로딩 스피너 표시 여부나 초기 fetch 실행 여부를 판다
...
// create 함수는 콜백 함수를 가짐
// : 콜백 함수는 set 설정 함수를 매개변수로 받고, state 제네릭 객체 타입을 반환
//    > 객체 반환시 함수의 구현부와 구분을 위해 ()로 감싼다.
export const useGlobalStore = create<GlobalState>((set) => ({
  // 속성값과 함수를 정의
  // - 속성값은 초기값 할당
  // - 함수는 구조 정의(해당 함수를 통해서 속성값 업데이트 할 수 있는 함수)
  categories: [],
  regions: [],
  isLoaded: false,
