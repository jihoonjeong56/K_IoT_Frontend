...
export const useReservationStore = create<ReservationState>((set) => ({
  ...
  fetchReservations: async (truckId) => {
    // const reservations:ReservationResponse = await getAllReservations(trunckId);
  ...
  fetchResrvationById: async (truckId, reservationId) => {
    ...
    try {
      ...
      return mapped; // undefined 를 방지한느 반환 - 해당 메서드의 반환 타입 Promise<Resercation | null>;
    } catch (e) {
      return null;
    }
  },
