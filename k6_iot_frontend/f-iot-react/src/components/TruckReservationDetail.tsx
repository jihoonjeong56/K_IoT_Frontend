...
function TruckReservationDetail() {
  ...
  const reservation = reservationList.find(
    (rservation) => rservation.timeSlot === selectedTimeSlot
  );
  if (!reservation) {
    return (
      <div className="panel">
  ...
  const handleCancel = async () => {
    ...
    setLoading(false);
    try {
      // await cancleReservation(reservation.id);
      // setActionMessage("예약이 취소되었습니다.")
      setActionMessage("취소기능이 구현되어있지 않습니다.");
    } catch (e) {
      console.error("cancel error: ", e);
      setActionMessage("예약 취소중 오류가 발생했습니다.");
    } finally {
      setLoading(false);
    }
  };
