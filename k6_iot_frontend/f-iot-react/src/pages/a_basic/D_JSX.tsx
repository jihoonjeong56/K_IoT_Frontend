...
/*
! JSX(TSX)
: JS(TS) 파일 내에서 HTML 과 유사한 마크업을 작성할 수 있도록 해주는JS 구문 확장 문법
...
- JSX 내애서 HTML 코드와 JS 코드를 분리 변환하려면 XML 문법을 준수
...
4) HTML 코드를 JSX로 변환시 주의 점
- 대부분 lowerCamelCase 사용
  : CSS 속성, on-메서드 (이벤트 핸들러) 등등등
...
function D_JSX() {
  ...
  return (
    ...
    */}
      <div>{welcomeMsg}</div>
      <div>{greeting("asdf")}</div>
      <div>
        이름: {userInfo.name}, 나이: {userInfo.age}
      </div>
      {/* 
      ? 요소의 이벤트 전달 시 
      : 코드를 해석하는 과정에서 싫행되지 않도록 '콜백 함수의 형태로 전달'
      - 익명 함수의 형태
