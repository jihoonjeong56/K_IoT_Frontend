...
/*
  cf) 컴포넌트(Component) : 리액틀의 구성요소(UI)

  ! Props(Properties): 컴포넌트의 속성
  : Props로 데이터 전달
    <ChildComponent name='asdf' />
    - 함수형 컴포넌트에서 데이터 받는 곳: 매개변수
      function ChildComponent({name:string}) { ... }
      @ 컴포넌트의 props는 객체로 전달(중괄호로 전달되기 때문)
...
function MultiProps(props: { name: string; colorProps: string }) {
  ...
  // > 하나의 객체에 전달 받거나 한 번에 각각의 요소 변수에 할당하는 구조 분해 할당도 가능
  // props.name = 'asdf'; - 읽기 전용 속성
...
// 3) 상태 변경이 필요하면 부모로 부터 콜백 함수르 ㄹ받아서 수행해야 함
...
function G_Props() {
  ...
  return (
    <Wrapper>
      <ChildComponent name="asd" />
      <ChildComponent2 name="123" />

      <MultiProps name="zxc" colorProps="lightblue" />
      <MultiProps2 name="ddd" colorProps="lightcoral" />
