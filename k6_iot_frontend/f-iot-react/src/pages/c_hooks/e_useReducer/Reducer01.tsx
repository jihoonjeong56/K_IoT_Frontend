...
//! === React 컴포넌트의 상태관리 (useState VS useReducer)

//! 1) useState
// : 리액트에서 가장 "기본적인 상태관리 Hooks"
// - 간단한 상태값 변경 시 사용
// - 컴포넌트 내부에서 직접 상태 처리
...
// - 상태 업데이트 로직을 외부에서 선언 가능 (재사용 가능)
// > 상테 업데이트 로직을 리듀서 함수(reducer)로 분리하여 관리 가능
// const [state, dispatch] = useReduver(reducer, 초기상태);
...
//  >> 상태변경 로직을 포함 (switch, case)하고, 이전 상태와 액션 객체를 인자로 받아 새로운 상태로 반환
...
// 복잡한 상태로직 (여러 조건, 단계적 변경): useReducer
