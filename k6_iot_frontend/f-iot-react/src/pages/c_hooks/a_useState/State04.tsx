...
function State04() {
  ...
  const handleSignUpSubmit = (e: React.FormEvent) => {
    ...
    //? -- 필수 입력값 체크
    if (!id.trim()) {
      tempError.id = "1d 를 입력해주세요 ";
      isValid = false;
    }
    if (!password.trim()) {
      tempError.password = "비밀번호를 입력해 주세요";
      isValid = false;
    }
    if (!confirmPassword.trim()) {
      tempError.confirmPassword = "비밀번호 확인을 입력해 주세요";
      isValid = false;
    }
    if (!email.trim()) {
      tempError.email = "이메일을 입력해 주세요";
      isValid = false;
    }
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSignUpSubmit}>
        ...
        <button type="submit">외원가입</button>
      </form>
    </div>
  );
}
