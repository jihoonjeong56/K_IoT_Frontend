...
// >> 상태값 변수명(user), set 상태값변수명(setUser)
// - 상태 변경 시 컴포넌트는 자동으로 재렌더링
// - 상태 업데이트는 비동기적으로 처리
// - 배열 구조분해 할당으로 사용 : [stat, setState] = useState(initialValue);
//  >> useState() 호출 시 인자로 전달되는 값은 state에 처음 할당될 기본값
// - 제네릭으로 <type>으로 타입 명시 가능
//  >> useState<User>({name:aaa, age:24})
...
function State01() {
  ...
  // initialValue: 초기값, 초기 상태를 설정 (생략 가능 - undefined값)
  ...
  // - 여러개의 상태 사용 시 , 관련 있느 훅끼리 묶어서 컴포넌트 상단에 배치 -> 가독성 향상
  ...
  return (
    <div>
      <p>카운트 클릭 횟수: {count}</p>
      <button onClick={handleUpClink}>카운트 증가</button>
      <button onClick={handleDownClink}>카운트 감소</button>
