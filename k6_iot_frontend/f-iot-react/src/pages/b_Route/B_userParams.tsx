...
//! useParams
// : React Router 에서 제공하는 훅(Hook) -> 기능
// - URL 주소에 적힌 변수(값)를 꺼내주는 도구

// EX) https://example.com/users/10
// : 사용자 번호(user ID : 10) - 프론트 엔드 화면을 10유저의 페이지로 출력
...
// >> 주소를 각각 다르게 인식하려면, useParams로 /users/:id중의 :id값을 추출

//? 기본 사용법
// 1) a태그 속성을 가진 요소로 화면 전환
//    : 페이지 이동 경로는 동적 데이터(추출될 값)를 포함한 경로 그대로
//    EX) `/practice/post/${posts.id}
...
//        params.추출할변수면 -> params.id


function B_userParams() {
  return <div>B_userParams</div>;
}
