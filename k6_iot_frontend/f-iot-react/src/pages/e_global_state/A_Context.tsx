...
// ex) 로그인 상태, 테마설정(다크/라이트 모드), 다국어 처리(언어 설정), UI 모달/알림 등
...
//    : 데이터를 공유 하고자 하는 범위에 대한 Context를 생성
//    - Provider(제공자) 와 Consumer(소비자) 컴포넌트를 포함
// 2) Provider - Condtext 객체로 부터 생성된 컴포넌트
...
// Provider 생성
// : 해당 컴포넌트 내부의 자식 & 자손 컴포넌트는 useContext훅을 통해 데이터 활용 가능
...
const ExampleHeader = () => {
  ...
  if (!user) {
    return <button onClick={() => setUser({ name: "aaa" })}>로그인</button>;
  }
  return (
    <div>
      <h4>프로젝트 헤더</h4>
...
function A_Context() {
  return (
    <div>
      ...
      {/* UserContext는 UserProvider 내에서 사용되지 않음 */}
      {/* <ExampleHeader /> */}
    </div>
  );
}
