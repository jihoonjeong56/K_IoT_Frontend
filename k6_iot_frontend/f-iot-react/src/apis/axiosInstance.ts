// : axios.create() 설정 파일
// - 반복되는 요청 설정 (URL, header 등)을 한 번에 정의하기 위해 axios인스턴스 생성
...
const API_BASE =
  import.meta.env.VITE_API_BASE || "http://localhast:8080/api/v1";
...
export const privateApi = axios.create({
  ...
  // 세션이나 쿠키 인증용: 쿠키나 인증 헤더 정보를 포함 시켜서 요청을 보냄
...
privateApi.interceptors.response.use(
  ...
  async (e) => {
    ...
    // alert("서버 요청 중 오류가 발샣하였습니다.");
    // e.config: Axios는 각 요청의 설정값(config)을 객체로 보관
    // - 실패한 요청의 원본 설정
