...
? q: 도시이름
  > 반드시 Capitalize 사용
    - 시작은 대문자, 기타문자는 소문자
    EX) Seoul, Busan, London, Tokyo 등등
...
document.getElementById("search-button").addEventListener("click", () => {
  ...
  // : input 창의 value 값을 getWeatherData에 전잘
  //? 입력값 검증 필요
...
async function getWeatherData(city) {
  ...
  try {
    ...
    document.getElementById(
      "weather-info"
    ).innerHTML = `<p>날씨 정보를 불러오는데 실패하였습니다. (${e.message})</p>`;
  }
}

function displayWeather(data) {
  //! 검색 결과 값(현재 도시의 날씨 데이터)을 브라우저에 출력
  const weatherInfoDiv = document.getElementById("weather-info");
  weatherInfoDiv.innerHTML = `<h2>${data.name}의 날씨 </h2>
